...
for i in range(j, len(lines)):
    ...
    numBoxes = int(instructions[0])
    boxes = stacks[int(instructions[1])-1][len(stacks[int(instructions[1])-1])-numBoxes:len(stacks[int(instructions[1])-1])]
    stacks[int(instructions[1])-1] = stacks[int(instructions[1])-1][:len(stacks[int(instructions[1])-1]) - numBoxes]
    stacks[int(instructions[2])-1].extend(boxes)
