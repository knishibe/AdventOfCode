...
def get_delete_size(node, delete_size, space_needed):
    if not node.is_dir:
        return delete_size
    else:
        current_size = int(node.get_size())
        if current_size < space_needed:
            return delete_size
        else:
            if delete_size is None:
                delete_size = current_size
            elif delete_size > current_size:
                delete_size = current_size
        for j in range(len(node.children)):
            current_size = get_delete_size(node.children[j], delete_size, space_needed)
            if delete_size > current_size:
                delete_size = current_size
        return delete_size
...
sumOfSize = get_delete_size(root, None, 30000000 - (70000000 - root.get_size()))
print("Delete Size: " + str(sumOfSize))
